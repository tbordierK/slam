function [H_t_bar,b_t_bar,mu_t_bar] = SEIF_motion_update(previous_H_t,previous_b_t,previous_mu_t,u_t,g,U_t,n)
...
% All the inputs can probably be refactored.
    ...
    %Compute gradient of the "motion" function g at (previous_mu_t,u_t). 
    A_t = eye(n); 

    %Pre-computations
    Sx = [eye(2),zeros(2,n-2)]';   
    delta_t = motion(previous_mu_t,u_t);
    
    psi_t = eye(n) + Sx*inv(eye(2)+inv(Sx'*A_t*Sx))*(Sx');
    H_prime = psi_t'*previous_H_t*psi_t;
    
    display(size(inv(U_t)+Sx'*H_prime*Sx));
    delta_H = H_prime*Sx*inv(inv(U_t)+Sx'*H_prime*Sx)*(Sx')*H_prime;
    delta_bar = motion(previous_mu_t,u_t);
